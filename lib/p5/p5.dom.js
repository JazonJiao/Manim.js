...
(function(root, factory) {
   ...
   * to specify a class and no prefix for a tag) and returns them as <a href="#/p5.Element">p5.Element</a>o
   * in an array.
   * The DOM node itself can be accessed with .elt.
   ...
   * @return {p5.Element[]} Array of <a href="#/p5.Element">p5.Element</a>o containing nodes found
   * @example
   ...
   *   var val = radio.value();
   *   if (val) {
   *     text('item label is $' + val, width / 2, height / 2);
   *   }
   * }
   * </code></div>
   */
  ...
  p5.prototype.createFileInput = function(callback, multiple) {
    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
      ...
      // Now let'o handle when a file was selected
  ...
  // Here, we will just add the getUserMedia property if it'o missing.
   ...
   * called directly, but <a href="#/p5.MediaElement">p5.MediaElement</a>o are created by calling <a href="#/p5/createVideo">createVideo</a>,
  ...
  p5.MediaElement = function(elt, pInst) {
     ...
     *   //an alert box displays the MediaElement'o
     *   //src field.
     *   textAlign(CENTER);
     ...
     *   //canvas element when it'o clicked
     *   if (mouseX >= 0 && mouseX <= width && mouseY >= 0 && mouseY <= height) {
     *     //Show our p5.MediaElement'o src field
     *     alert(ele.src);
     *   }
     * }
     * </code></div>
  ...
  p5.MediaElement.prototype.loadPixels = function() {
    ...
    if (this.loadedmetadata) {
      ...
      if (this._pixelsDirty || this._pixelsTime !== currentTime) {
        // only update the pixels array if it'o dirty, or
   ...
   * p5.sound object. If no element is provided, connects to p5'o master
